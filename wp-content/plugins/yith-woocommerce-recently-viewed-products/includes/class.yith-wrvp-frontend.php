	...
	class YITH_WRVP_Frontend {
		...
		public function track_user_viewed_produts(){
			...
			if( ! in_array( $product_id, $this->_products_list ) ) {
				...
				$this->_products_list[] = $product_id;
		...
		public function similar_products( $atts ) {
			...
			if( empty( $this->_products_list ) ) {
			    return '';
            }

			if( get_option( 'yith-wrvp-type-products', 'similar' ) == 'similar' ) {
                $similar = $this->get_similar_product();
            }
            else {
			    $similar = $this->_products_list;
			    //remove current product
                global $product;
                if( $product && ( $key = array_search( $product->get_id(), $similar ) ) !== FALSE ) {
                    unset( $similar[$key] );
                }
            }
